...
ListModel {
    ...
    function loadItemsData() {
        ...
        db.transaction(
            function(tx) {
                tx.executeSql('CREATE TABLE IF NOT EXISTS menuDB(cid INTEGER primary key, title TEXT, image TEXT, style TEXT, slotQml TEXT, backColor TEXT, foreColor TEXT)');
        ...
        db.transaction(
            function(tx) {
                ...
                while (index < menuGridModel.count && menuGridModel.get(index).style != "ADD_RECT") {
                    ...
                    tx.executeSql('INSERT INTO menuDB VALUES(?,?,?,?,?,?,?)', [item.cid, item.title, item.image, item.style, item.slotQml, item.backColor, item.foreColor]);
                    index++;
                }
            }
        )
